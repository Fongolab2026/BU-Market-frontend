...
import { demandesService } from '../services/demandesService.js'
...
export function DemandesPage() {
  ...
  useEffect(() => {
    let active = true
    demandesService.list().then((result) => {
      if (active) {
        setRequests(result)
        setLoading(false)
      }
...
const updateStatus = (id, nextStatus) => {
    const apply = nextStatus === 'approved' ? demandesService.approve(id) : demandesService.reject(id)
    apply.then((updated) => {
