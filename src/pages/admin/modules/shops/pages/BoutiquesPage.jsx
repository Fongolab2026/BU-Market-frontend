...
import { boutiquesService } from '../services/boutiquesService.js'
...
export function BoutiquesPage() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      boutiquesService.list({ ...filters, page }).then((response) => {
        setData(response)
        setLoading(false)
      })
    }, 150)
  ...
  const handleDelete = async () => {
    ...
    await boutiquesService.remove(deleteTarget.id)
  ...
  const handleSubmit = async (event) => {
    ...
    setSubmitting(true)
    await boutiquesService.update(editTarget.id, form)
    setSubmitting(false)
    setEditTarget(null)
    setReloadKey((key) => key + 1)
