...
import { parametresService } from '../services/parametresService.js'
...
export function SettingsPage() {
  ...
  useEffect(() => { parametresService.get().then(setSettings) }, [])
  ...
  const save = async () => {
    setSaving(true)
    await parametresService.update(settings)
    setSaving(false)
  ...
  const addCategory = async (event) => {
    ...
    const categories = await parametresService.addCategory(newCategory.trim())
  ...
  const saveRename = async () => {
    ...
    const categories = await parametresService.renameCategory(editingId, editingName.trim())
    setSettings((current) => ({ ...current, categories }))
    setEditingId(null)
    setEditingName('')
  ...
  const removeCategory = async () => {
    ...
    const categories = await parametresService.removeCategory(deleteTarget.id)
    setSettings((current) => ({ ...current, categories }))
