...
export function ListeProduitsPage() {
  ...
  useEffect(() => {
    let active = true
    adminService.listProducts().then((result) => {
      if (active) {
        setProducts(result)
        setLoading(false)
      }
  ...
  useEffect(() => {
    adminService.listShops({ perPage: 50 }).then((result) => setShops(result.results))
  }, [])
  ...
  const toggleStatus = async (product) => {
    ...
    await adminService.updateProductStatus(product.shopId, product.id, nextStatus)
    const updated = await adminService.listProducts()
    setProducts(updated)
  ...
  const handleSubmit = async (event) => {
    ...
    setSubmitting(true)
    if (editing) {
      await adminService.updateProduct(editing.shopId, editing.id, form)
      toast.success('Produit modifié.')
    } else {
      await adminService.createProduct(form.shopId, form)
  ...
  const handleDelete = async () => {
    ...
    await adminService.deleteProduct(deleteTarget.shopId, deleteTarget.id)
    const updated = await adminService.listProducts()
    setProducts(updated)
