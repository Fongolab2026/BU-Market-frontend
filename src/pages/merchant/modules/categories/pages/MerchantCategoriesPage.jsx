...
import { Edit, Eye, EyeOff, Package, Plus, Search, X } from 'lucide-react'
...
export function MerchantCategoriesPage() {
  ...
  const openCreate = () => { setForm({ name: '', description: '', icon: '📦', active: true }); setModal('create') }
  const openEdit = (category) => { setForm({ name: category.name, description: category.description, icon: category.icon, active: category.active }); setModal(category.id) }
  ...
  const toggleStatus = async (category) => {
    ...
    toast.success(category.active ? 'Catégorie désactivée.' : 'Catégorie activée.')
    await reload()
  }
      ...
      <section className="card">
        <div className="flex flex-col gap-3 border-b border-base-200 p-4 sm:flex-row sm:items-center sm:justify-between">
          <label className="relative block w-full sm:max-w-sm">
            ...
            <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Rechercher une catégorie…" className="input input-bordered w-full pl-10 pr-3 text-sm placeholder:text-base-content/40 focus:border-brand focus:ring-2 focus:ring-brand/20" />
          </label>
          <div className="flex items-center gap-2 flex-wrap">
            <span className="px-3 py-1.5 text-xs font-semibold rounded-full bg-brand/10 text-brand">{categories.length} catégories</span>
            <span className="px-3 py-1.5 text-xs font-semibold rounded-full bg-emerald-50 text-emerald-700">{activeCount} actives</span>
          </div>
        </div>
        ...
        <div className="p-4">
          {loading ? (
            ...
            <div className="p-12 text-center">
              ...
              <p className="mt-1 text-sm text-base-content/45">Créez votre première catégorie.</p>
            </div>
          ) : (
            <>
                  ...
                  .map((category) => (
                    <article key={category.id} className="group rounded-2xl border border-base-200 bg-white overflow-hidden transition-all duration-300 hover:border-brand/40 hover:shadow-xl hover:-translate-y-1">
                      ...
                      <div className="p-4 space-y-3">
                        ...
                        <p className="text-sm text-base-content/60 line-clamp-2">{category.description || 'Aucune description'}</p>
                        <div className="flex items-center gap-2 pt-2 border-t border-base-100">
                          <button onClick={() => toggleStatus(category)} className={`inline-flex flex-1 items-center justify-center gap-1.5 rounded-lg px-2 py-1.5 text-xs font-semibold transition ${category.active ? 'bg-base-100 text-base-content/60 hover:bg-base-200' : 'bg-emerald-50 text-emerald-700 hover:bg-emerald-100'}`}>
                            {category.active ? <> <EyeOff size={14} /> Désactiver </> : <> <Eye size={14} /> Activer </>}
                          </button>
                          <button onClick={() => openEdit(category)} className="rounded-lg p-2 text-base-content/40 transition hover:bg-brand/10 hover:text-brand" aria-label={`Modifier ${category.name}`}>
                            <Edit size={16} />
                          </button>
              ...
              <ConfirmDialog
                ...
                message={deleteTarget ? `La catégorie « ${deleteTarget.name} » sera supprimée.` : ''}
...
function CategoryForm({ isCreate, form, setFormField, submitting, onSubmit, onClose }) {
  const icons = ['🍎', '👕', '🪵', '🏠', '💄', '📱', '📚', '🎮', '🎨', '🧸', '🛠️', '🌱']
    ...
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/50 p-4 animate-in fade-in zoom-in-95 duration-200">
      <form onSubmit={onSubmit} className="card w-full max-w-lg p-5">
        <div className="flex items-start justify-between gap-3">
          <div>
            <h2 className="text-lg font-bold text-base-content">{isCreate ? 'Nouvelle catégorie' : 'Modifier la catégorie'}</h2>
            <p className="mt-1 text-sm text-base-content/55">{isCreate ? 'Cette catégorie sera disponible pour vos produits.' : 'Mettez à jour les informations de la catégorie.'}</p>
          </div>
        ...
        <div className="mt-4 grid gap-3">
          <label className="block">
            ...
            <input value={form.name} onChange={(event) => setFormField('name', event.target.value)} placeholder="Ex : Alimentation" className="input input-bordered w-full text-sm placeholder:text-base-content/40 focus:border-brand focus:ring-2 focus:ring-brand/20" />
          </label>
          <label className="block">
            <span className="mb-1.5 block text-sm font-semibold text-base-content/80">Description</span>
            <textarea value={form.description} onChange={(event) => setFormField('description', event.target.value)} placeholder="Description courte..." rows={3} className="textarea textarea-bordered w-full text-sm placeholder:text-base-content/40 focus:border-brand focus:ring-2 focus:ring-brand/20 resize-none" />
          </label>
          <label className="block">
            <span className="mb-1.5 block text-sm font-semibold text-base-content/80">Icône</span>
            <div className="flex flex-wrap gap-2">
              {icons.map((icon) => (
                <button
                  key={icon}
                  type="button"
                  onClick={() => setFormField('icon', icon)}
                  className={`flex size-10 items-center justify-center rounded-xl border-2 text-2xl transition ${form.icon === icon ? 'border-brand bg-brand/10' : 'border-base-200 hover:border-brand/50'}`}
                >
                  {icon}
                </button>
              ))}
          ...
          <label className="flex items-center gap-3">
            <input type="checkbox" checked={form.active} onChange={(event) => setFormField('active', event.target.checked)} className="checkbox checkbox-primary" />
            <span className="text-sm font-medium text-base-content/80">Catégorie active</span>
          </label>
        </div>
        ...
        <div className="mt-5 flex justify-end gap-2">
          ...
          <button type="submit" disabled={!form.name.trim() || submitting} className="rounded-lg bg-brand px-4 py-2 text-sm font-semibold text-white transition hover:bg-brand-soft disabled:cursor-not-allowed disabled:opacity-50">
