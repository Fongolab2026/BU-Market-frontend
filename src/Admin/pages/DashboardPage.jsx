...
const queueTarget = (type) => type === 'Boutique' ? '/admin/boutiques?status=pending' : type === 'Produit' ? '/admin/produits' : '/admin/avis?status=hidden'
  ...
  return (
    <>
      <PageHeader
        ...
        description="Suivi des utilisateurs, boutiques, produits et éléments à traiter."
        action={
          <span className="inline-flex items-center gap-2 rounded-full border border-base-200 bg-white px-3 py-1.5 text-xs font-medium text-base-content/60">
            <Clock3 size={15} /> Mis à jour à 10:24
          </span>
        }
      />
      ...
      <section className="mt-5 grid gap-4 xl:grid-cols-[minmax(0,1.4fr)_minmax(360px,0.9fr)]">
        <article className="card p-4 sm:p-5">
          ...
          <div className="mt-5 flex h-44 items-end justify-between gap-2 sm:gap-4" aria-label="Graphique d'activité hebdomadaire">
            {data.weeklyActivity.map((value, index) => (
              <div key={weekDays[index]} className="flex h-full flex-1 flex-col items-center justify-end gap-2">
                <span className="text-xs font-semibold text-base-content/55">{value}</span>
                <div className="w-full max-w-10 rounded-t-md bg-brand transition-all" style={{ height: `${(value / maxActivity) * 100}%` }} />
        ...
        <article className="card p-4 sm:p-5">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="font-semibold text-base-content">À modérer</h2>
              <p className="mt-1 text-sm text-base-content/55">Les derniers éléments reçus.</p>
            </div>
            <Link to="/admin/boutiques?status=pending" className="text-sm font-semibold text-brand hover:text-brand-soft">Tout voir</Link>
          </div>
            ...
            {data.moderationQueue.map((item) => (
              <Link key={item.id} to={queueTarget(item.type)} className="flex gap-3 py-3 first:pt-0 transition hover:bg-base-100">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-1 text-xs text-base-content/50">{item.type} - {item.owner}</p>
                  <p className="mt-1 text-xs text-base-content/40">{item.date}</p>
                </div>
              </Link>
            ))}
          </div>
      ...
      <section className="card mt-5 p-4 sm:p-5">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-base-content/55">Les dernières actions réalisées sur la plateforme.</p>
          </div>
          <span className="hidden bg-base-200 p-2 text-base-content/55 sm:block">
            <Activity size={18} />
          </span>
        </div>
        <div className="mt-4 grid gap-px bg-base-200 lg:grid-cols-2 xl:grid-cols-4">
          {data.activity.map((entry) => {
            ...
            return (
              <div key={entry.id} className="bg-white p-4">
                ...
                <p className="mt-1 text-xs leading-5 text-base-content/55">{entry.description}</p>
                <p className="mt-3 text-xs font-medium text-base-content/45">{entry.time}</p>
              </div>
            )
          })}
        </div>
